...
public class excelDataManager {
	...
	public Object[][] getData(Method m) throws EncryptedDocumentException, IOException
	{
		...
		File file = new File(System.getProperty("user.dir")+"\\InputData\\TestData.xlsx");
			...
			int i = NoofRowsexe.get(rc);
			 Map<Object, Object> datamap = new HashMap<>();
		      for (int j = 0; j < sheet.getRow(0).getPhysicalNumberOfCells()-1; j++) {
		        datamap.put(sheet.getRow(0).getCell(j+1).getStringCellValue(), sheet.getRow(i).getCell(j+1).getStringCellValue());
		      }
		      data[rc][0] = datamap;
		      rc++;
		}
		...
		for(int i=1;i<=sheet.getPhysicalNumberOfRows();i++) {
			for(int j=0;j<sheet.getRow(0).getPhysicalNumberOfCells();j++) {
				if(sheet.getRow(j).getCell(0).getStringCellValue().equals(User)) {
					...
					UserNameandPass.put(sheet.getRow(0).getCell(1).getStringCellValue(), sheet.getRow(j).getCell(1).getStringCellValue());
					UserNameandPass.put(sheet.getRow(0).getCell(2).getStringCellValue(), sheet.getRow(j).getCell(2).getStringCellValue());
